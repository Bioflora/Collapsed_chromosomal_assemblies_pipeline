...
# here if you just put one number e.g. 40, which means split depth in three classes 0, 0-40 and > 40 
perl Util/stat.depth_region_interval.pl region.tsv.gz.split 30,120 > region.tsv.gz.split.stat
# $F[-1] > 10 indicate only capture utg > 100000 bp
perl -anle 'print $_ if ($F[3] > 0.2 && $F[-1] >= 10)' region.tsv.gz.split.stat |cut -f 1 > z.collapsed.contig.100k.lst
cd ..
